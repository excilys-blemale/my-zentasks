...
object Task {
  ...
  implicit object TaskFormat extends Format[Task] {
    def reads(json: JsValue): Task = throw new UnsupportedOperationException()

    def writes(t: Task): JsValue = JsObject(List("id" -> JsNumber(t.id.get), "folder" -> JsString(t.folder), "project" -> JsNumber(t.project), "title" -> JsString(t.title), "done" -> JsBoolean(t.done), "dueDate" -> JsString(t.dueDate.map(_.toString) getOrElse ("")), "assignedTo" -> JsString(t.assignedTo.getOrElse(""))))
  }
